...
class Lib_Tool
{
	...
	/**
	 * 获取网页的内容。貌似和file_get_contents速度差不多, 不过比 file_get_contents 好很多（稳定）
...
//		curl_setopt($ch, CURLOPT_TIMEOUT, $timeout); // 貌似有个地方是这样写的，也没有下面一行
	 ...
	 * @return multitype:
	 */
	public static function cnChunkSplit($str, $len) {
	...
	/**
	 * 获取document(该页面上)中的所有图片，并保存到指定的目录
	 * 该函数依赖 createdDirectory() 函数
	 * @param string $text 
	 * @param string $folderPath 需要保存到哪里，绝对路径，后面是否带 / 没有关系
	 */
	public static function getAllImages($text, $folderPath) { // TODO 域名
//		$text = '<img src="http://www.xiqiyanyan.com/images/small_02.png" align="middle" />';
		$imgPattern = '/<img[^<>]*?src="([^<>]*?\/[^<>]*?\.(jpg|gif|png))"/i';
		preg_match_all($imgPattern, $text, $imgMatches); // print_r($imgMatches);exit;
		$pattern = '/<img[^<>]*?src=(http:\/\/[^<>]*?\/[^<>]*?\.(jpg|gif|png))[^<>]*?>/i'; // 整个下来貌似有点小bug
		preg_match_all($pattern, $text, $matches); // print_r($matches[1]);
		$imgUrlList = array_merge($imgMatches[1],$matches[1]); // print_r($imgUrlList);
		foreach ($imgUrlList as $imgUrl) {
			$imgUrl = 'http://xiqiyanyan.com/' . $imgUrl; // TODO 传进来的网页主机头
...
/*			if (false === ($fileOpen = fopen($folderPath . $localImageUrl, 'w'))) {
	...
	/**
	 * 获取来源地址（get the http_referer url）;  即是哪个页面跳转过来的
...
//	}
	...
	/**
	 * 判断是否为手机号，check the string is a mobile number (有问题不能用）
	 * @param string $string
	 * @return number(integer 0: false，不是; 1:true，是)
	 */
...
//					$folderPath = preg_replace('/\\\/', '/', $folderPath);  // 注意这里的写法
	 ...
	 * @param string $src
	 * @param string $dest
	 * @param string $apiUrl  google 翻译接口url，固定地址
	 */
	public static function googleTranslate($text, $src = 'zh-CN', $dest = 'en', $apiUrl = 'http://translate.google.com/translate_a/t') {
		$googleTranslateUrl = $apiUrl . '?client=t&text=' . $text . '&sl=' . $src . '&tl=' . $dest;
//		$contents = file_get_contents($googleTranslateUrl);
		$contents = self::postUrl($googleTranslateUrl);
		return $contents;
	}
